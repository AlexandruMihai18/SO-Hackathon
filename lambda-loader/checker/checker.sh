...
#
# Checker script for SO Hackathon Dec 2022
#
...
test_harness() {
    ...
    for CLIENT_NUM in $(seq 1 ${_CLIENT_COUNT}); do
        ...
        timeout 1s "${CLIENT_BIN}" "$_LIB_NAME" $_FN_NAME $_ARG 1> "${CLIENT_OUTPUT}" 2> "${CLIENT_OUTPUT_ERR}" &
